  ...
  return (
    <div className="hidden md:block" ref={notificationRef}>
      ...
      >
        ...
        <div className="md:w-full text-xs sm:text-base sm:px-5 pt-2 flex gap-2 overflow-x-auto w-4/5 mx-auto pb-2 sm:pb-0 ">
          {options.map((option) => (
            <div
              ...
              className={`sm:px-3 px-2 py-2 my-auto cursor-pointer ${notifState === option.value
        ...
        >
          {(options.find((opt) => opt.value === notifState)).data.map((notification) => (
            <div
              key={notification.id}
            ...
            >
              <div className="flex gap-3 pb-3 border-b-[1px] border-grey">
                ...
                >
                  <FaDotCircle className={`${notification.status === "waiting" ?
                    "text-yellow-400"
                ...
                <div>
                  <div className="w-auto">
                    ...
                    >
                      from :
                      <span className=""> {notification.sender.firstName + " " + notification.sender.lastName} </span>
                    </label>
                  </div>
              ...
              <div>
                <TruncatedMessage message={`${notification.message}`} maxLength={90} textClass="" />
              </div>
              <div className="grid grid-flow-col justify-stretch gap-3 justify-self-end text-center">
                {notification.status === "waiting" ? (
                  <>
                    <button className="bg-secondary hover:bg-secondaryAlternative active:scale-95 p-2 
                      rounded-lg transition-all duration-75 ease-in text-white"
                      onClick={() => { sendResponseInvitation(notification, "accepted") }}>Accept
                    </button>
                    <button className="bg-primary hover:bg-primaryAlternative active:scale-95 w-full p-2 
                      rounded-lg transition-all duration-75 ease-in text-white"
                      onClick={() => sendResponseInvitation(notification, "rejected")}>Reject
                    </button>
                  </>
                ) : (
                  notification.status === "accepted" ? (
                    <div className="font-bold mt-2">Invitation has been accepted</div>
                  ) : (
                    <div className="font-bold mt-2">Invitation has been rejected</div>
                  )
                )}
